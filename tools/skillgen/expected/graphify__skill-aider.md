...
if doc_files:
    ...
    Path('.graphify_doc.json').write_text(
...
_suppress = set()
_doc_json = Path('.graphify_doc.json')
if _doc_json.exists():
    try:
...
ast = json.loads(Path('.graphify_ast.json').read_text())
doc = json.loads(Path('.graphify_doc.json').read_text(encoding='utf-8'))
...
"
rm -f .graphify_detect.json .graphify_extract.json .graphify_ast.json .graphify_doc.json .graphify_semantic.json .graphify_analysis.json .graphify_labels.json; find . -maxdepth 1 -name '.graphify_chunk_*.json' -delete 2>/dev/null
...
Before the merge step, save the old graph: `cp .graph/graph.json .graphify_old.json`
