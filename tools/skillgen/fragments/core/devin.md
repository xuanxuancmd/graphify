---
name: graphify
description: "Use for any question about a codebase, its architecture, file relationships, or project content — especially when .graph/ exists, where the question should be treated as a graphify query first. Turns any input (code, docs, papers, images, videos) into a persistent knowledge graph with god nodes, community detection, and query/path/explain tools."
...
/graphify <path> --neo4j                              # generate .graph/cypher.txt for Neo4j
...
1. **Persistent graph** - relationships are stored in `.graph/graph.json` and survive across sessions. Ask questions weeks later without re-reading everything.
...
detect = json.loads(Path('.graph/.graphify_detect.json').read_text())
video_files = detect.get('files', {}).get('video', [])
prompt = os.environ.get('GRAPHIFY_WHISPER_PROMPT', 'Use proper punctuation and paragraph breaks.')

transcript_paths = transcribe_all(video_files, initial_prompt=prompt)
print(json.dumps(transcript_paths))
" > .graph/.graphify_transcripts.json
```
...
if code_files:
    ...
    Path('.graph/.graphify_ast.json').write_text(json.dumps(result, indent=2))
...
- Load `total_words` and file counts from `.graph/.graphify_detect.json`
...
if cached_nodes or cached_edges or cached_hyperedges:
    Path('.graph/.graphify_cached.json').write_text(json.dumps({'nodes': cached_nodes, 'edges': cached_edges, 'hyperedges': cached_hyperedges}))
else:
    Path('.graph/.graphify_cached.json').unlink(missing_ok=True)
Path('.graph/.graphify_uncached.txt').write_text('\n'.join(uncached))
print(f'Cache: {len(all_files)-len(uncached)} files hit, {len(uncached)} files need extraction')
"
```

Only dispatch subagents for files listed in `.graph/.graphify_uncached.txt`. If all files are cached, skip to Part C directly.
...
- Check that `.graph/.graphify_chunk_N.json` exists on disk — this is the success signal
...
chunks = sorted(glob.glob('.graph/.graphify_chunk_*.json'))
...
Path('.graph/.graphify_semantic_new.json').write_text(json.dumps({
...
new = json.loads(Path('.graph/.graphify_semantic_new.json').read_text()) if Path('.graph/.graphify_semantic_new.json').exists() else {'nodes':[],'edges':[],'hyperedges':[]}
uncached = [line for line in Path('.graph/.graphify_uncached.txt').read_text().splitlines() if line]
...
cached = json.loads(Path('.graph/.graphify_cached.json').read_text()) if Path('.graph/.graphify_cached.json').exists() else {'nodes':[],'edges':[],'hyperedges':[]}
new = json.loads(Path('.graph/.graphify_semantic_new.json').read_text()) if Path('.graph/.graphify_semantic_new.json').exists() else {'nodes':[],'edges':[],'hyperedges':[]}
...
Path('.graph/.graphify_semantic.json').write_text(json.dumps(merged, indent=2))
...
extraction = json.loads(Path('.graph/.graphify_extract.json').read_text())
detection  = json.loads(Path('.graph/.graphify_detect.json').read_text())
...
wrote = to_json(G, communities, '.graph/graph.json')
if not wrote:
    print('ERROR: refused to shrink .graph/graph.json (fewer nodes than the existing graph). Run a full rebuild to be safe.')
    raise SystemExit(1)
report = generate(G, communities, cohesion, labels, gods, surprises, detection, tokens, 'INPUT_PATH', suggested_questions=questions)
Path('.graph/GRAPH_REPORT.md').write_text(report)
...
Path('.graph/.graphify_analysis.json').write_text(json.dumps(analysis, indent=2))
...
Read `.graph/.graphify_analysis.json`. For each community key, look at its node labels and write a 2-5 word plain-language name (e.g. "Attention Mechanism", "Training Pipeline", "Data Loading").
...
extraction = json.loads(Path('.graph/.graphify_extract.json').read_text())
detection  = json.loads(Path('.graph/.graphify_detect.json').read_text())
analysis   = json.loads(Path('.graph/.graphify_analysis.json').read_text())
...
Path('.graph/GRAPH_REPORT.md').write_text(report)
Path('.graph/.graphify_labels.json').write_text(json.dumps({str(k): v for k, v in labels.items()}))
...
wrote = to_json(G, communities, '.graph/graph.json', community_labels=labels)
if not wrote:
    print('ERROR: refused to shrink .graph/graph.json (fewer nodes than the existing graph). Run a full rebuild to be safe.')
...
extraction = json.loads(Path('.graph/.graphify_extract.json').read_text())
analysis   = json.loads(Path('.graph/.graphify_analysis.json').read_text())
labels_raw = json.loads(Path('.graph/.graphify_labels.json').read_text()) if Path('.graph/.graphify_labels.json').exists() else {}
...
n = to_obsidian(G, communities, '.graph/obsidian', community_labels=labels or None, cohesion=cohesion)
print(f'Obsidian vault: {n} notes in .graph/obsidian/')

to_canvas(G, communities, '.graph/obsidian/graph.canvas', community_labels=labels or None)
print('Canvas: .graph/obsidian/graph.canvas - open in Obsidian for structured community layout')
print()
print('Open .graph/obsidian/ as a vault in Obsidian.')
...
if G.number_of_nodes() > NODE_LIMIT:
    ...
    if meta.number_of_nodes() > 1:
        ...
        to_html(meta, meta_communities, '.graph/graph.html', community_labels=labels or None, member_counts=member_counts)
    ...
    to_html(G, communities, '.graph/graph.html', community_labels=labels or None)
...
Run this before Step 9 (cleanup) so `.graph/.graphify_labels.json` is still available.
...
n = to_wiki(G, communities, '.graph/wiki', community_labels=labels or None, cohesion=cohesion, god_nodes_data=gods)
print(f'Wiki: {n} articles written to .graph/wiki/')
print('  .graph/wiki/index.md  ->  agent entry point')
"
```
...
G = build_from_json(json.loads(Path('.graph/.graphify_extract.json').read_text()), directed=IS_DIRECTED)
to_cypher(G, '.graph/cypher.txt')
print('cypher.txt written - import with: cypher-shell < .graph/cypher.txt')
"
```
...
to_svg(G, communities, '.graph/graph.svg', community_labels=labels or None)
...
to_graphml(G, communities, '.graph/graph.graphml')
...
```bash
python3 -m graphify.serve .graph/graph.json
```
...
detection = json.loads(Path('.graph/.graphify_detect.json').read_text())
result = run_benchmark('.graph/graph.json', corpus_words=detection['total_words'])
...
detect = json.loads(Path('.graph/.graphify_detect.json').read_text())
extract = json.loads(Path('.graph/.graphify_extract.json').read_text())
...
cost_path = Path('.graph/cost.json')
if cost_path.exists():
...
if [ ! -f .graph/.graphify_python ]; then
    ...
    mkdir -p .graph
    "$PYTHON" -c "import sys; open('.graph/.graphify_python', 'w').write(sys.executable)"
fi
```
...
Path('.graph/.graphify_incremental.json').write_text(json.dumps(result))
...
result = json.loads(open('.graph/.graphify_incremental.json').read()) if Path('.graph/.graphify_incremental.json').exists() else {}
...
# Load existing graph
existing_data = json.loads(Path('.graph/graph.json').read_text())
G_existing = json_graph.node_link_graph(existing_data, edges='links')

# Load new extraction
new_extraction = json.loads(Path('.graph/.graphify_extract.json').read_text())
...
old_data = json.loads(Path('.graph/.graphify_old.json').read_text()) if Path('.graph/.graphify_old.json').exists() else None
new_extract = json.loads(Path('.graph/.graphify_extract.json').read_text())
...
Skip Steps 1-3. Load the existing graph from `.graph/graph.json` and re-run clustering:
...
data = json.loads(Path('.graph/graph.json').read_text())
...
to_json(G, communities, '.graph/graph.json')
...
```bash
"$(cat .graph/.graphify_python)" -m graphify save-result --question "QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
