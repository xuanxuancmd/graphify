...
Markdown sidecars under `.graph/converted/`. Office files (`.docx`,
...
Every extracted file is fingerprinted by content hash. Re-runs skip unchanged files entirely — only new or modified files go through extraction again. The cache lives in `.graph/cache/`.
