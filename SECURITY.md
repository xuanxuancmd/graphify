...
| Path traversal in MCP server | `security.validate_graph_path()` resolves paths and requires them to be inside `.graph/`. Also requires the `.graph/` directory to exist. |
