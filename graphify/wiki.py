...
def _community_article(
    ...
    for nid in top_nodes:
        ...
        lines.append(f"- **{node_label}**（{degree} 个连接）{src_str}")
    remaining = len(nodes) - len(top_nodes)
    if remaining > 0:
        lines.append(f"- *...该社区还有 {remaining} 个节点*")
    lines.append("")

    lines += ["## 关系", ""]
    if cross:
        for other_label, count in cross[:12]:
            lines.append(f"- {_md_link(other_label, resolver)}（{count} 个共享连接）")
    else:
        lines.append("- 未检测到强跨社区连接")
    lines.append("")
    ...
    lines += ["---", "", f"*graphify 知识 wiki 的一部分。参见 {_md_link('index', resolver)} 以导航。*"]
    return "\n".join(lines)
...
def _god_node_article(G: nx.Graph, nid: str, labels: dict[int, str], node_community: dict[str, int] | None = None, resolver: dict[str, str] | None = None) -> str:
    ...
    lines: list[str] = []
    lines += [f"# {node_label}", ""]
    lines += [f"> God node · {G.degree(nid)} 个连接 · `{src}`", ""]

    if community_name:
        lines += [f"**所属社区：** {_md_link(community_name, resolver)}", ""]
...
def _index_md(
    ...
    lines: list[str] = [
        "# 知识图谱索引",
        "",
        "> 由 graphify 自动生成。从这里开始——读社区文章建立上下文，再深入 god nodes 看细节。",
        "",
        f"**{total_nodes} 个节点 · {total_edges} 条边 · {len(communities)} 个社区**",
        "",
        "---",
        "",
        "## 社区",
    ...
    for cid, nodes in sorted(communities.items(), key=lambda x: -len(x[1])):
        label = labels.get(cid, f"社区 {cid}")
        lines.append(f"- {_md_link(label, resolver)} — {len(nodes)} 个节点")
    lines.append("")

    if god_nodes_data:
        lines += ["## God Nodes", "（连接数最多的概念——承重的抽象）", ""]
        for node in god_nodes_data:
            lines.append(f"- {_md_link(node['label'], resolver)} — {node['degree']} 个连接")
        lines.append("")

    lines += [
        "---",
        "",
        "*由 [graphify](https://github.com/safishamsi/graphify) 生成*",
    ]
    return "\n".join(lines)
...
def to_wiki(
    ...
    if not communities:
        raise ValueError(
            "communities dict 为空——拒绝清空 wiki/。"
            "请先运行 `graphify extract .` 或 `graphify cluster-only .`。"
        )
    ...
    if _kept_total < _orig_total:
        print(
            f"wiki: 丢弃了 {_orig_total - _kept_total} 个图中已不存在的陈旧节点 ID"
            f"（剩余 {len(communities)} 个社区）",
            file=_sys.stderr,
        )

    if not communities:
        raise ValueError(
            "所有社区节点 ID 都已陈旧——图中一个都不存在了。"
            "请重新运行 `graphify extract .` 以重新生成 .graphify_analysis.json。"
        )
