"""Deterministic "work memory" reflection over .graph/memory/.
...
The artifact lands at ``.graph/reflections/LESSONS.md`` rather than inside the wiki
...
def _render_bucket(out: list[str], data: dict[str, Any], k: int) -> None:
    ...
    if preferred:
        out += [f"**首选来源** —— 已被 ≥{k} 次有用结果印证；从这里开始查。", ""]
        for e in preferred:
            out.append(f"- `{e['node']}`（{e['n']}× 有用）")
        out.append("")
    if tentative:
        out += [f"**暂定** —— 有用次数不足 {k} 次；使用前请核验。", ""]
        for e in tentative:
            out.append(f"- `{e['node']}`（{e['n']}× 有用）")
        out.append("")
    if contested:
        ...
        for e in contested:
            day = e["last"][:10]
            verdict = ("势均力敌" if e["verdict"] == "even"
                       else f"近期倾向 **{e['verdict']}**")
            out.append(
                f"- `{e['node']}` —— {e['pos']}× 有用，{e['neg']}× "
                f"死胡同/已更正 → {verdict}"
                + (f"（最近 {day}）" if day else ""))
        out.append("")
    if dead_ends:
        out += ["**已知死胡同** —— 查不出结果；别再重复推导。", ""]
        for d in dead_ends:
...
def render_lessons_md(agg: dict[str, Any]) -> str:
    ...
    out: list[str] = [
        "# 经验",
        "",
        f"_由 `graphify reflect` 从 .graph/memory/ 中的 {agg['total']} 条会话"
        f"{'记忆' if agg['total'] == 1 else '记忆'}自动生成。"
        "确定性过程，无 LLM。用于建立整体认知——使用前请核验；"
        "若代码已变更，请重新审视死胡同。_",
        "",
        "## 概要",
        "",
        f"- {c['useful']} 有用 · {c['dead_end']} 死胡同 · "
        f"{c['corrected']} 已更正 · {c['unmarked']} 未标记",
        "",
        "## 经验",
        "",
    ]
...
def _resolve_source_path(src: str, graph_path: Path) -> Path | None:
    ...
    live in ``<root>/.graph/`` (so its own dir is not the root) or directly
    ...
    root *first* — graph.json's parent's parent for the ``.graph`` layout,
