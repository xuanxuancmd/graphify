...
/graphify <path> --falkordb                           # generate .graph/cypher.txt for FalkorDB
...
**Fast path — existing graph:** Before doing anything else, check whether `.graph/graph.json` exists. The expected location is `.graph/graph.json` relative to the **current working directory** (i.e. the project root where you are running commands). If it exists AND the user's request is a natural-language question about the codebase (e.g. "How does X work?", "What calls Y?", "Trace the data flow through Z") and NOT an explicit rebuild command (`--update`, `--cluster-only`, or a bare path/URL that implies fresh extraction): **skip Steps 1–5 entirely and jump straight to `## For /graphify query`.** Run `graphify query "<question>"` immediately. Do not run detect. Do not check corpus size. Do not ask the user to narrow. The graph is already built — use it.
...
Path('.graph/.graphify_detect.json').write_text(json.dumps(result, ensure_ascii=False), encoding="utf-8")
print(f'Detected {result["total_files"]} files')
'@ | & (Get-Content .graph\.graphify_python) -
```
...
- If `total_words` > 2,000,000 OR `total_files` > 500: show the warning. Then compute the top 5 first-level subdirectories by file count:
  ...
  - Filter out any path that starts with `scan_root + "/.graph/"` to exclude converted sidecars.
...
if code_files:
    ...
    Path('.graph/.graphify_ast.json').write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
...
if cached_nodes or cached_edges or cached_hyperedges:
    Path('.graph/.graphify_cached.json').write_text(json.dumps({'nodes': cached_nodes, 'edges': cached_edges, 'hyperedges': cached_hyperedges}, ensure_ascii=False), encoding="utf-8")
else:
...
- Check that `.graph/.graphify_chunk_NN.json` exists on disk — this is the success signal
...
new = json.loads(Path('.graph/.graphify_semantic_new.json').read_text(encoding="utf-8")) if Path('.graph/.graphify_semantic_new.json').exists() else {'nodes':[],'edges':[],'hyperedges':[]}
uncached = [line for line in Path('.graph/.graphify_uncached.txt').read_text(encoding="utf-8").splitlines() if line]
...
cached = json.loads(Path('.graph/.graphify_cached.json').read_text(encoding="utf-8")) if Path('.graph/.graphify_cached.json').exists() else {'nodes':[],'edges':[],'hyperedges':[]}
new = json.loads(Path('.graph/.graphify_semantic_new.json').read_text(encoding="utf-8")) if Path('.graph/.graphify_semantic_new.json').exists() else {'nodes':[],'edges':[],'hyperedges':[]}
...
Path('.graph/.graphify_semantic.json').write_text(json.dumps(merged, indent=2, ensure_ascii=False), encoding="utf-8")
print(f'Extraction complete - {len(deduped)} nodes, {len(all_edges)} edges ({len(cached["nodes"])} from cache, {len(new.get("nodes",[]))} new)')
'@ | & (Get-Content .graph\.graphify_python) -
```
Clean up temp files: `Remove-Item -Force -ErrorAction SilentlyContinue .graph\.graphify_cached.json, .graph\.graphify_uncached.txt, .graph\.graphify_semantic_new.json`
...
extraction = json.loads(Path('.graph/.graphify_extract.json').read_text(encoding="utf-8"))
detection  = json.loads(Path('.graph/.graphify_detect.json').read_text(encoding="utf-8"))
...
if not wrote:
    print('ERROR: refused to shrink .graph/graph.json (existing graph has more nodes; #479).')
    print('If this shrink is intentional (you deleted files), re-run a full build with --force.')
    raise SystemExit(1)
report = generate(G, communities, cohesion, labels, gods, surprises, detection, tokens, 'INPUT_PATH', suggested_questions=questions)
Path('.graph/GRAPH_REPORT.md').write_text(report, encoding="utf-8")
analysis = {
...
Path('.graph/.graphify_analysis.json').write_text(json.dumps(analysis, indent=2, ensure_ascii=False), encoding="utf-8")
...
extraction = json.loads(Path('.graph/.graphify_extract.json').read_text(encoding="utf-8"))
detection  = json.loads(Path('.graph/.graphify_detect.json').read_text(encoding="utf-8"))
analysis   = json.loads(Path('.graph/.graphify_analysis.json').read_text(encoding="utf-8"))
...
Path('.graph/GRAPH_REPORT.md').write_text(report, encoding="utf-8")
Path('.graph/.graphify_labels.json').write_text(json.dumps({str(k): v for k, v in labels.items()}, ensure_ascii=False), encoding="utf-8")
...
- If `--obsidian-dir <path>` was also given, pass it via `--dir`. Otherwise defaults to `.graph/obsidian`.
...
detect = json.loads(Path('.graph/.graphify_detect.json').read_text(encoding="utf-8"))
extract = json.loads(Path('.graph/.graphify_extract.json').read_text(encoding="utf-8"))
...
'@ | & (Get-Content .graph\.graphify_python) -
Remove-Item -Force -ErrorAction SilentlyContinue .graph\.graphify_detect.json, .graph\.graphify_extract.json, .graph\.graphify_ast.json, .graph\.graphify_doc.json, .graph\.graphify_semantic.json, .graph\.graphify_analysis.json
Get-ChildItem .graph -Filter '.graphify_chunk_*.json' -File -ErrorAction SilentlyContinue | Remove-Item -Force
